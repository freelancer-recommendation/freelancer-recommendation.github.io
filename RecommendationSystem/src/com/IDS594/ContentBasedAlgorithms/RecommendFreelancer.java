/**
 * This Program recommends freelancers to a given project
 */
...
public class RecommendFreelancer {
	...
	public static void main(String[] args) {
		...
		try{
			...
			for (int pid = 0; pid < projectId; pid++){
				...
				for (int uid = 0; uid<freelancerId; uid++){
					double value = 0;
					double required_score = 0;
					for (int sid = 0; sid < max_limit; sid++){
						if (project_Skill_Matrix[pid][sid]!=0){
							required_score++;
						}
					}
					for (int ssid=0; ssid < max_limit; ssid++){
						for (int skillId = 0; skillId < max_limit; skillId++){
							if ((project_Skill_Matrix[pid][ssid] == user_Skill_Matrix[uid][skillId])&&(project_Skill_Matrix[pid][ssid]!=0)){
								value++;
								break;
							}
						}
					...
					for (int tempInd = 0; tempInd < max_limit; tempInd++){
						if (scores[tempInd] < value){
							...
							recommendation[tempInd] = uid;
							break;
						}
					}					
				}
				result.write("Project" + pid + "(" + projectMatrix[pid] + "): ");
				for (int val = 0 ; val < max_limit; val++){
					result.write("Freelancer" + recommendation[val] + "(" +freelancerMatrix[recommendation[val]] + ")[" + scores[val] + "], ");
				}
				result.write("\n");
			}
			result.close();
			...
			System.out.println("Exception in main function of RecommendFreelancer"+e);
		}
